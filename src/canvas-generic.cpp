...
bool CanvasGeneric::reset()
{
    ...
    if (!supports_gl2()) {
        Log::error("it needs OpenGL(ES) version >= 2.0 to run"
...
void CanvasGeneric::visible(bool visible)
{
    if (visible && !offscreen_)
        native_state_.visible(visible);
}

void CanvasGeneric::clear()
{
...
Canvas::Pixel CanvasGeneric::read_pixel(int x, int y)
{
    uint8_t pixel[4];
...
void CanvasGeneric::write_to_file(std::string &filename)
{
...
bool CanvasGeneric::should_quit()
{
    return native_state_.should_quit();
}

void CanvasGeneric::resize(int width, int height)
{
    resize_no_viewport(width, height);
    glViewport(0, 0, width_, height_);
}

unsigned int CanvasGeneric::fbo()
{
    return fbo_;
}
...
bool CanvasGeneric::supports_gl2()
{
...
bool CanvasGeneric::resize_no_viewport(int width, int height)
{
...
bool CanvasGeneric::do_make_current()
{
...
bool CanvasGeneric::ensure_gl_formats()
{
...
void CanvasGeneric::release_fbo()
{
    if (fbo_) {
...
const char * CanvasGeneric::get_gl_format_str(GLenum f)
{
    const char *str;
